...
export const communityService = {
   ...
   * Update a post
   */
  async updatePost(postId: string, data: Partial<CreatePostData> | FormData): Promise<ApiResponse<Post>> {
    try {
   ...
   * Delete a comment
   */
  async deleteComment(commentId: string): Promise<ApiResponse<{ success: boolean; message: string }>> {
    try {
      const response = await api.delete<ApiResponse<{ success: boolean; message: string }>>(
        `/comments/${commentId}`,
      );
  ...
  async getFollowStats(
    ...
    try {
      const response = await api.get<ApiResponse<{ followersCount: number; followingCount: number }>>(
